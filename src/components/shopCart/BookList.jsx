        ...
        <>
            <tr>
                ...
                <td><div className="table__value">
                    <button className="price__btn" onClick={() => setCount(count > 1 ? count - 1 : 1)}><span className='price__minus'>-</span></button>
                    <input type="number" className="price__value" min='1' value={count} required />
                    <button className="price__btn" onClick={() => setCount(count + 1)}><span className='price__plus'>+</span></button>
                </div></td>
                <td><div className="table__price">{'$' + Number(book.price.replace('$', '') * count).toFixed(2)}</div></td>
                <td><button className="table__delete" onClick={() => deleteBook(id)}>&times;</button></td>
            </tr>
